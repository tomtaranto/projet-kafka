...
case class ViewAggregate(
  ...
  def increment(_id:Int,movie: String,category: String) = {
    category match{
      case "start_only" => {
        this.copy(_id,movie, this.categorie1_count + 1, this.categorie2_count, this.categorie3_count)

      }
      case "half" => {
        this.copy(_id,movie, this.categorie1_count, this.categorie2_count + 1, this.categorie3_count)
      }
      case _ => {
        this.copy(_id,movie, this.categorie1_count, this.categorie2_count + 1, this.categorie3_count + 1)
      }
    }
